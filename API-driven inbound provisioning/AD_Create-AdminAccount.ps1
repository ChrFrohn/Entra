<#
.SYNOPSIS
    Creates an admin account in Active Directory using API-driven provisioning and SCIM bulk upload.

.DESCRIPTION
    This script uses Microsoft Graph SCIM API-driven provisioning to create a new admin account in 
    Active Directory based on an existing Entra ID user. The script:
    - Authenticates using Managed Identity
    - Retrieves the current user information from the Microsoft Graph via REST API
    - Uses SCIM bulk upload to create a new admin account with "Admin" prefix
    - Creates the account in Active Directory with appropriate admin settings
    
    Designed for execution in Azure environments with Managed Identity configured.
    The admin account will be created with proper naming conventions in Active Directory.
...
.EXAMPLE
    .\AD_Create-AdminAccount.ps1 -UserPrincipalNameOrObjectId "user@christianfrohn.dk"
    Creates an admin account based on the specified user

.EXAMPLE
    .\AD_Create-AdminAccount.ps1 -UserPrincipalNameOrObjectId "12345678-1234-1234-1234-123456789012"
...
.NOTES
    ...
    - API-driven inbound provisioning configured from Entra ID to Active Directory
    - Provisioning agent installed and configured for Active Directory synchronization
    ...
    - SynchronizationData-User.Upload: Required for API-driven inbound provisioning operations
...
# Create SCIM JSON payload for On-Premises admin user creation
        $json = @"
    {
...
Write-Output "SUCCESS: Admin account '$JSONexternalId' queued for creation in AD based on user '$UserPrincipalName'"
